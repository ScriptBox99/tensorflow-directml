...
class CropAndResizeImageGradShapeHelper : public ShapeHelper {
 public:
...
class DmlCropAndResizeImageGradKernel : public DmlKernel {
 public:
  using InitHelper = CropAndResizeGradImageInitHelper;

  explicit DmlCropAndResizeImageGradKernel(DmlKernelConstruction* ctx,
                                           const InitHelper* init_helper) {
    const TensorShape& input_gradient_shape = ctx->GetInputTensorShape(0);
    ...
    const uint32_t batch_size = output_gradient_shape.dim_size(1);
    ...
    auto result = dml::RoiAlignImageGrad(
...
#define DML_REGISTER_KERNEL(type)                       \
  REGISTER_KERNEL_BUILDER(                              \
      ...
      DmlKernelWrapper<DmlCropAndResizeImageGradKernel, \
                       CropAndResizeImageGradShapeHelper>);
